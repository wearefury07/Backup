...
public class WeiHuWarpChannel : MonoBehaviour
{

    public static WeiHuWarpChannel Channel;

#if UNITY_WEBGL
    public WebSocket cSocket { get; set; }
    private static string httpRequestServer = "https://192.168.1.95:8181/api/";
    private static string prod_server = "ws://192.168.1.95:8877/websocket";
    private static int prod_port = 8888;
#else
    private static string httpRequestServer = "https://192.168.1.95:8181/api/";
    private static string prod_server = "wss://192.168.1.95:8877/websocket";
    private static int prod_port = 8888;
#endif
    ...
    {
        if (WeiHuWarpClient.currentState == WarpConnectionState.RECOVERING && isConnected == false)
        {
            if (reset)
            {
                client_socket.Close();
    ...
    public void socket_connect(Action onConnected = null)
    {
        ...
        if (WeiHuWarpClient.currentState == WarpConnectionState.CONNECTING && this.isConnected == false)
        {
            if (client_socket == null)
            {
                if (connectCoroutine != null)
...
#endif
                    this.isConnected = true;
                    WeiHuWarpClient.currentState = WarpConnectionState.CONNECTED;
    ...
    public void socket_close()
    {
        ...
        setReceive = false;
        isConnected = false;
        WeiHuWarpClient.currentState = (int)WarpConnectionState.DISCONNECTED;
    }
    ...
    public void socket_recv()
    {
        if (client_socket == null)
        {
            if (WeiHuWarpClient.currentState != WarpConnectionState.RECOVERING)
            {
                serverCloseSocket();
            }
        }
        ...
        WeiHuWarpClient.wc.ResponseData(reply);


        if ((client_socket == null || client_socket.isConnected == false) && WeiHuWarpClient.currentState != WarpConnectionState.DISCONNECTED)
        {
            Debug.Log("++++++++++++++++++++ DISCONNECT FROM IN GAME ++++++++++++++++++++++++++");
            Debug.Log("set status to RECOVERING");
            WeiHuWarpClient.currentState = WarpConnectionState.RECOVERING;
            socket_close();
            isConnected = false;
        }
    }
    ...
    public void onLostConnection(int status)
    {
        ...
        socket_disconnect();
        WeiHuWarpClient.currentState = WarpConnectionState.DISCONNECTED;
        try
        {

            if (WeiHuWarpClient.wc == null || WeiHuWarpClient.wc.sessionId != 0)
    ...
    void Update()
    {
        if (isConnected == false && WeiHuWarpClient.currentState == WarpConnectionState.CONNECTING)
        {
        ...
        if ((WeiHuWarpClient.currentState == WarpConnectionState.CONNECTED && (diff > 4)) || ((WeiHuWarpClient.currentState == WarpConnectionState.RECOVERING) && (diff > 2)))
        {
            if (WeiHuWarpClient.currentState == WarpConnectionState.RECOVERING)
            {
    ...
    public void SwichServerToTest(Toggle toggle)
    {
        if (toggle.isOn)
        {
			...
			//warp_host = "192.168.1.95";
			httpRequest = "https://"+ warp_host + ":8686/api/";
#elif UNITY_WEBGL
            warp_host = "ws://192.168.1.95:8877/websocket";
#endif
            warp_port = 8888;
        }
        else
